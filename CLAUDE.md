...
### 7.1 PM (Project Manager) 권한 재정의

**담당:** Claude Haiku 4.5 (비용최적) | **이상적:** Claude Haiku 4.5

**역할:** 세션 관리자 (조율자 + 기록자 + 감시자)
...
### 7.2 Documentation Manager 역할 재정의

**담당:** Claude Haiku 4.5 (비용최적) | **이상적:** Claude 4.5 Sonnet

**역할:** 검증자 + 구조 전문가 (단순 반복 작업에서 해방, 고부가가치 작업 집중)

**핵심 책임 (호출 빈도: Phase 전환 시 또는 주 1회)**

✅ **정합성 검증**
- 정본 문서(`01~05`, `CLAUDE.md`) 간 모순 점검
- 콘텐츠 SSOT(`docs/contents/`) ↔ 코드 동기화 확인
- 링크 유효성 검사
- 버전 히스토리 일관성 점검
...
**호출 빈도:**
- 이전: 매 세션 (10회/10세션)
- 개선: Phase 전환 시만 (2~3회/10세션, 70% 감소)
...
### 7.5 External Auditor + Architect (겸임 역할)

**담당:** Claude 4.5 Sonnet

**역할:** 외부감사관 겸 아키텍트 (거버넌스 감사 + 기술 설계 확정)
...
1. **Phase 3 → Phase 4 전환 시**
   - Phase 4 진입 가능성 검증
   - 기술 부채 점검 및 아키텍처 정합성 최종 확인
   - 산출물: Phase 4 진입 승인 또는 차단 사유 보고서

2. **Contact 백엔드 설계 확정 시**
   - Supabase/Resend 연동 구현 착수 전 설계 검토
   - DB 스키마 확정 및 API 명세 승인
   - 산출물: Contact 백엔드 설계 승인서

**권장 호출 지점 (RECOMMENDED):**
3. Session 20 최종 QA 시 (RACI: C)
4. 배포 전 성능 최적화 시

**특징:**
- 이슈 발생 시 임시 호출(ad-hoc) + 로드맵 필수 지점에서 의무 투입
- 비용 최적화: Claude 4.6 Opus 대신 4.5 Sonnet 활용 (토큰 비용 40% 절감)
- D-024 참조

---

### 7.6 Analyst (보조 역할, 필요시만)

**담당:** Claude 4.5 Sonnet (현재 Architect 역할과 병행 불가)
