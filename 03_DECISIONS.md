...
### D-019: 아카이브 정책 수립 (Deprecated 리포트 관리)

```
결정:
  - `docs/reports/`에는 최근 3개월 이내의 활성 리포트만 보관
  - 3개월 이상 경과한 분석/감사 리포트는 `docs/_archive/reports/` 이동
  - Deprecated 콘텐츠(`docs/contents/track-record.md`)는 보관용으로 명시 후 더 이상 수정하지 않음

이유:
  - Session 19.6 Doc Manager 검수 결과, `docs/reports/`에 오래된 분석 리포트(2026-02-06, 2026-02-12)가 혼재
  - 최신 리포트 검색/참조 시 혼란 야기
  - 아카이브 정책이 없어 문서 스프롤 위험

대안:
  - 옵션 A (채택): 명시적 아카이브 정책 + 정기 정리
  - 옵션 B (기각): 현상 유지 → 문서 스프롤 가속화

변경 가능 여부:
  - 높음 (3개월 기준은 조정 가능)
  - 아카이브 폴더 구조는 운영 중 확장 가능

일시: 2026-02-15 (Session 19.6 문서 검수 후)
```
